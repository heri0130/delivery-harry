<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            StoreOrderMgmt # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            StoreOrderMgmt
        </v-card-title >        
        ...
        <v-card-text>
            ...
            <Number label="MenuQty" v-model="value.menuQty" :editMode="editMode" :inputUI="''"/>
            <Number label="OrderId" v-model="value.orderId" :editMode="editMode" :inputUI="''"/>
            <Number label="PayId" v-model="value.payId" :editMode="editMode" :inputUI="''"/>
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/storeOrderMgmts'), this.value)
                        } else {
